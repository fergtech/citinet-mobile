...
// 3-bucket FEATURE_TYPES). This is that missing screen: every unread
// notification in one place, across all 6 real types, each dismissible on
// its own tap rather than only in bulk per feature.
export default function NotificationsScreen() {
  ...
  function handlePress(n: HubNotification) {
    if (!session) return;
    // Optimistic removal — same "act on the tap, don't wait on the network"
    // convention as this app's like/RSVP toggles, just with no rollback:
    // worst case a failed mark-read leaves it reappearing on next load,
    // which is a fine failure mode for "dismiss a notification."
  ...
  return (
    <ThemedView style={styles.flex}>
      ...
      {loading && notifications.length === 0 && <ActivityIndicator style={styles.spinner} />}
